  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'standard conditions' do
        ...
        it {
          expect(function).to contain_mysql_user('testuser@localhost').with(
            require: 'Class[Mysql::Server::Root_password]',
          )
        }

        it {
          expect(function).to contain_mysql_grant('testuser@localhost/*.*').with(
        ...
        it {
          expect(function).to contain_file('mysqlbackup.sh').with(
        ...
        it {
          expect(function).to contain_file('/tmp/mysql-backup').with(
            ensure: 'directory',
          )
        }
        ...
        it 'does not mention triggers by default because file_per_database is false' do
          expect(function).to contain_file('mysqlbackup.sh').without_content(
            %r{.*triggers.*},
          )
        end
        ...
        it 'has 25 days of rotation' do
          # MySQL counts from 0
          expect(function).to contain_file('mysqlbackup.sh').with_content(%r{.*ROTATE=24.*})
        end

        it 'has a standard PATH' do
          expect(function).to contain_file('mysqlbackup.sh').with_content(%r{PATH=/usr/bin:/usr/sbin:/bin:/sbin:/opt/zimbra/bin})
        end
      end
      ...
      context 'with file per database' do
        ...
        it 'loops through backup all databases' do
          expect(function).to contain_file('mysqlbackup.sh').with_content(%r{.*SHOW DATABASES.*})
        end
        ...
        context 'with compression disabled' do
          ...
          it 'loops through backup all databases without compression #bzcat' do
            expect(function).to contain_file('mysqlbackup.sh').without_content(%r{.*bzcat -zc.*})
          end
        end
