  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      it 'does not notify the import sql exec if no sql script was provided' do
        expect(function).to contain_mysql_database('test_db').without_notify
      end

      it 'subscribes to database if sql script is given' do
        params['sql'] = sql
        expect(function).to contain_mysql_database('test_db')
        expect(function).to contain_exec('test_db-import').with_subscribe('Mysql_database[test_db]')
      end

      it 'onlies import sql script on creation if not enforcing' do
        params.merge!('sql' => sql, 'enforce_sql' => false)
        expect(function).to contain_exec('test_db-import').with_refreshonly(true)
      end

      it 'imports sql script on creation' do
        params.merge!('sql' => sql, 'enforce_sql' => true)
        # ' if enforcing #refreshonly'
        expect(function).to contain_exec('test_db-import').with_refreshonly(false)
        # 'if enforcing #command'
        expect(function).to contain_exec('test_db-import').with_command('cat /tmp/test.sql | mysql test_db')
      end
      ...
      it 'imports sql script with custom command on creation' do
        ...
        expect(function).to contain_exec('test_db-import').with_refreshonly(false)
        # if enforcing #command
        expect(function).to contain_exec('test_db-import').with_command('zcat /tmp/test.sql | mysql test_db')
      end

      it 'imports sql scripts when more than one is specified' do
        params['sql'] = ['/tmp/test.sql', '/tmp/test_2.sql']
        expect(function).to contain_exec('test_db-import').with_command('cat /tmp/test.sql /tmp/test_2.sql | mysql test_db')
      end

      it 'does not create database' do
        params.merge!('ensure' => 'absent', 'host' => 'localhost')
        expect(function).to contain_mysql_database('test_db').with_ensure('absent')
        expect(function).to contain_mysql_user('testuser@localhost').with_ensure('absent')
      end

      it 'creates with an appropriate collate and charset' do
        params.merge!('charset' => 'utf8', 'collate' => 'utf8_danish_ci')
        expect(function).to contain_mysql_database('test_db').with('charset' => 'utf8',
                                                                  'collate' => 'utf8_danish_ci')
      end

      it 'uses dbname parameter as database name instead of name' do
        params['dbname'] = 'real_db'
        expect(function).to contain_mysql_database('real_db')
      end

      it 'uses tls_options for user when set' do
        params['tls_options'] = ['SSL']
        expect(function).to contain_mysql_user('testuser@localhost').with_tls_options(['SSL'])
      end

      it 'uses grant_options for grant when set' do
        params['grant_options'] = ['GRANT']
        expect(function).to contain_mysql_grant('testuser@localhost/test_db.*').with_options(['GRANT'])
      end
      ...
      [
        ...
        it "fails when provided '#{path}' as a value to the 'sql' parameter" do
          params['sql'] = [path]
          expect(function).to raise_error(Puppet::PreformattedError, %r{The file '#{Regexp.escape(path)}' is invalid. A valid file path is expected.})
        end
      end
      ...
      [
        ...
        it "succeeds when provided '#{path}' as a value to the 'sql' parameter" do
          params['sql'] = [path]
          expect(function).to contain_exec('test_db-import').with_command("cat #{path} | mysql test_db")
        end
      end
      ...
      [
        ...
        it "fails when provided '#{name}' as a value to the 'name' parameter" do
          params['name'] = name
          expect(function).to raise_error(Puppet::PreformattedError, %r{The database name '#{name}' is invalid.})
        end
      end
      ...
      [
        ...
        it "succeeds when the provided '#{name}' as a value to the 'dbname' parameter" do
          params['dbname'] = name
          expect(function).to contain_mysql_database(name)
        end
      end
    end
  end
