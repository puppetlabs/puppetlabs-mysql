Puppet::Type.type(:database_user).provide(:mysql) do
  ...
  def create
    ...
    dbh.select_db('mysql').query("create user '%s' identified by PASSWORD '%s'" % [ @resource[:name].sub("@", "'@'"), @resource.value(:password_hash) ])
    dbh.reload
  ...
  def destroy
    dbh = Puppet::Type.type(:database_user).provider(:mysql).connect
    dbh.select_db('mysql').query("drop user '%s'" % @resource.value(:name).sub("@", "'@'"))
    dbh.reload
